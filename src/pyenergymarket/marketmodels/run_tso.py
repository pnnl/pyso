...
class EgretProvider(DataProvider):
    ...
    def get_model(self, daterange: pd.DatetimeIndex) -> ModelData:
        ...
        # Convert datetime objects to formatted strings for time_keys
        daterange_str = [
                ...
                for _etype, etype_dict in elem_dict.items():
                    for _ename, eentries in etype_dict.items():
                        if isinstance(eentries, dict):
                            # Check if this is a time series entry
                            is_time_series = (
...
class TSO:
    ...
    def add_market(self, market_name, market_timing, freq=None):
        # Create market with appropriate parameters
    ...
    def initialize_steps(self):
        ...
        for mtype in self.markets.keys():
            ...
            # In the (likely) case that these aren't the same
            # We will run an initial DA market and pass results to RT
    ...
    def simulate(self):
        ...
        while not horizon_reached:
            ...
            for market_name in self.markets.keys():
                ...
                if market_cleared:
                    # Log market clearing time in appropriate units
                    sim_time_in_units = self.simulation_time / self.time_scaling
                    logger.info(
                        f"{market_name} cleared at simulation time {sim_time_in_units} "
                        f"{self.time_unit}s"
                    )
...
def execute_generic(options, time_unit="hour"):
    ...
    tso = TSO(options)
    # Edit the market timing here
    # TODO: pull from a file or alternate input
    market_timing = {
        "states": {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--start_time", help="Start time in YYYYmmddHHMM format", default="202005010000"
    )
    parser.add_argument(
        "-e", "--end_time", help="End time in YYYYmmddHHMM format", default="202005080000"
    )
    parser.add_argument(
        "-f",
        "--filename",
        help="Name (with path) to egret model_data file",
    ...
    parser.add_argument(
        "--da_only", help="If included, will only run the day-ahead market", action="store_true"
    )
    ...
    parser.add_argument(
        ...
        "--time_resolution",
        type=int,
        help="The simulation time resolution in units of seconds.",
        default=None,
    )
    ...
    # Log the selected command line options
    cmd_line = rev_parser.get_pretty_command_line_invocation()
