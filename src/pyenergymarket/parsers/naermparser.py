...
def is_time_series(elem):
    if not isinstance(elem, dict):
        return False
    return elem.get('data_type', None) == 'time_series' and \
           'time_series_uid' in elem and \
           'scale_factor' in elem
...
def assign_ts_values(elem_ts: dict, ts: dict, ts_uid_to_idx: dict):
    ...
    if isinstance(ts_uid, str):
        elem_ts['values'] = np.array(ts_scale_factor * \
                               ts['values'][:,ts_uid_to_idx[ts_uid]], \
                               dtype=float).tolist()
        ...
        for i in range(len(ts_uid)):
            elem_ts['values'] += np.array(ts_scale_factor[i] * \
                                    ts['values'][:,ts_uid_to_idx[ts_uid[i]]], \
                                    dtype=float)
...
def enforce_p_min_p_max_consistency(md_elem_gen: dict):
    ...
    for gen_uid, gen in md_elem_gen.items():
        if gen['generator_type'] != 'renewable' or \
           gen.get('model_type', None) not in ('HY', 'WT', 'PV'):
            continue
        if is_egret_time_series(gen.get('p_max', None)) and \
           is_egret_time_series(gen.get('p_min', None)):
    ...
    for elem_type in md_elem:
        ...
        for elem_k, elem in md_elem[elem_type].items():
            ...
            for prop_k, prop in elem.items():
                if is_egret_time_series(prop) and \
                   len(prop.get('values', [])) == 1:
...
class TimeSeries:
    ...
    def _unix_tmstamp_to_idx_back(self, unix_tmstamp: int):
        ...
        idx = next((n - 1 - j for j, x in enumerate(reversed(self._timestamp)) if \
                    x <= unix_tmstamp), None)
        if idx is None:
