            ...
            for k, v in g_dict.items():
                ...
                if isinstance(v, dict):
                    ...
                    if ("startup_cost" in k) or ("startup_fuel" in k):
                        ...
                        err_msg = f"flatten_distributed_generators: unknown data type {type(v)}"
                        raise TypeError(f"{err_msg} for property {k}: {v}.")
...
def sum_properties(a: Union[float, dict], b: Union[float, dict]):
    ...
    if isinstance(a, dict):
        return {
            ...
            "values": [i + j for i, j in zip(a["values"], b["values"])],
        }
    else:
        return a + b
...
def get_total_load(md: ModelData):
    out = None
    for _l, l_dict in md.elements("load"):
        if out is None:
            out = l_dict["p_load"]
        else:
...
def get_total_gen(md: ModelData):
    out = None
    for _g, g_dict in md.elements("generator"):
        if out is None:
            out = g_dict["pg"]
        else:
...
def merge_model_data(md1: ModelData, md2: ModelData, ts_keep="md2"):
    ...
    Args:
        ...
        ts_keep (str, optional): If md1 and md2 have common time keys,
        specifies which time series to keep.
    Returns:
            ...
            for elem_name in md1["elements"][element].keys():
                for elem_char, elem_dict in md1["elements"][element][elem_name].items():
                    ...
                    if isinstance(elem_dict, dict):
                        if (
                            "data_type" in elem_dict.keys()
                            and elem_dict["data_type"] == "time_series"
                        ):
                            md1_values = deepcopy(elem_dict["values"])
                            md2_values = deepcopy(
                                md2["elements"][element][elem_name][elem_char]["values"]
                            )
                            # Handle any overlapping time
                            md1_values, md2_values = index_merge(
                                md1_values, md2_values, idx, ts_keep
                            )
    ...
    def merge_system(md1: dict, md2: dict, idx: Union[int, None], ts_keep: str):
        ...
        md1_times, md2_times = (
            deepcopy(md1["system"]["time_keys"]),
            deepcopy(md2["system"]["time_keys"]),
        )
        # Handle any overlapping time
    ...
    # Check if there are any shared times. Depending on ts_keep,
    # this will select the start/end index
...
def get_networkx_graph(md: ModelData) -> nx.Graph:
    ...
    G: nx.Graph = nx.from_pandas_edgelist(df, source="from_bus", target="to_bus", edge_attr=True)
